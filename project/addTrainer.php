...
if (isset($_GET["id"])) {
    $pmkTrainerIdNo = (int) htmlentities($_GET["id"], ENT_QUOTES, "UTF-8");

    $query = 'SELECT fldFirstName, fldLastName';
    $query .= 'FROM tblTrainer WHERE pmkTrainerIdNo = ?';

    $results = $thisDatabase->select($query, array($pmkTrainerIdNo), 1, 0, 0, 0, false, false);
...
// form. Note it is best to follow the same order as declared in section 1c.
    $pmkTrainerIdNo = (int) htmlentities($_POST["hidTrainerIdNo"], ENT_QUOTES, "UTF-8");
    if ($pmkTrainerIdNo > 0) {
        $update = true;
    }
...
//
        ...
        try {
            ...
            if ($update) {
                $query = 'UPDATE tblTrainer SET ';
            } else {
                $query = 'INSERT INTO tblTrainer SET ';
            }
            ...
            if ($update) {
                $query .= 'WHERE pmkTrainerIdNo = ?';
                $data[] = $pmkTrainerIdNo;

                if ($_SERVER["REMOTE_USER"] == 'ejosephs') {
...
//
        ...
        <form action="<?php print $phpSelf; ?>"
            ...
            <fieldset class="wrapper">
                ...
                <input type="hidden" id="hidTrainerIdNo" name="hidTrainerIdNo"
                       value="<?php print $pmkTrainerIdNo; ?>"
                       >
